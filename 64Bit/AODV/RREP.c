...
int fn_NetSim_AODV_GenerateRREP(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	rrep->PrefixSz = 0;
	rrep->Type = 2;
	rrep->LastAddress = AODV_DEV_IP_COPY(pstruEventDetails->nDeviceId);
...
/**
This function processess a route reply.
...
int fn_NetSim_AODV_ProcessRREP(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	if(rrep->DestinationIPaddress == AODV_DEV_IP(pstruEventDetails->nDeviceId))
		return 0;
	...
	if(!IP_COMPARE(AODV_DEV_IP(pstruEventDetails->nDeviceId),rrep->OriginatorIPaddress))
	{
...
{
	AODV_RREP* rrep = (AODV_RREP*)pstruEventDetails->pPacket->pstruNetworkData->Packet_RoutingProtocol;
	if(AODV_CHECK_ROUTE_FOUND(rrep->OriginatorIPaddress) && pstruEventDetails->pPacket->nDestinationId/*For hello packet*/)
	{
		NetSim_EVENTDETAILS pevent;
		NETSIM_ID port;
