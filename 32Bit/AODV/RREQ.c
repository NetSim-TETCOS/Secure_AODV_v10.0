...
NetSim_PACKET* fn_NetSim_AODV_GenerateRREQ(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	if(!nDestSeq)
		rreq->JRGDU[4] = '1';
	rreq->OriginatorIPAddress = AODV_DEV_IP_COPY(pstruEventDetails->nDeviceId);
...
int fn_NetSim_AODV_ProcessRREQ(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	if(!IP_COMPARE(AODV_DEV_IP(pstruEventDetails->nDeviceId),rreq->DestinationIPAddress))
	{
...
int fn_NetSim_AODV_ForwardRREQ(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	rreq->LastAddress = AODV_DEV_IP_COPY(pstruEventDetails->nDeviceId);
